"""
Helper-oriented PAVA mixture decomposition experiments.

This module is intentionally separate from pava.py and pava2.py.  It keeps
the pava2 search structure but changes the PAVA shape statistic to a
right-end-weighted ISE on [x0, 0].  The default weight is gamma=2, normalized
to have average weight 1 on the evaluated grid interval.
"""
...
from glissade.pava import (
	_random_state_label,
	_ecdf_on_grid,
	_gap_T,
	_concave_projection_on_interval,
	_convex_projection_on_interval,
	_convex_gap_T,
	_as_valid_cdf,
	_cdf_violation_stats,
	_ise_on_grid,
	_flat_tail_stat,
	_bootstrap_quantile_level,
	_sample_from_cdf,
	_build_grid,
	_run_shape_alpha_bracket,
	_merge_alpha_probe_tables,
	_run_cdf_alpha_boundary,
	tol_bootstrap,
	feasible_at_alpha,
	alpha_minimize,
	mixture_sanity_check,
)


TOOL_TAIL_PROXY_CALIBRATION = "human_clean_2026-08-06"
...
class _DerivedSearchContext:
	def __init__(
		...
		*,
		...
		self.mix_tail_quantile_cap = (
			None if mix_tail_quantile_cap is None else float(mix_tail_quantile_cap)
		)
		if self.mix_tail_quantile_cap is not None:
			if not np.isfinite(self.mix_tail_quantile_cap) or not (0.0 < self.mix_tail_quantile_cap < 1.0):
				raise ValueError("mix_tail_quantile_cap must be None or a finite value in (0, 1).")
			self.mix_tail_quantile_cap_x = float(np.quantile(self.x_mix, self.mix_tail_quantile_cap))
		else:
			self.mix_tail_quantile_cap_x = None
	...
	def _apply_count_filter(self):
		...
		quantile_ok = np.ones(len(candidate_x), dtype=bool)
		if self.mix_tail_quantile_cap_x is not None:
			quantile_ok = candidate_x <= self.mix_tail_quantile_cap_x
		if self.use_tail_count_cutoff:
			count_ok = (mix_tail_counts >= self.min_mix_tail) & (alt_tail_counts >= self.min_alt_tail)
			count_ok = count_ok & quantile_ok
			if not np.any(count_ok):
				self.setup_error = {
					...
					'mix_tail_quantile_cap': (
						None if self.mix_tail_quantile_cap is None else float(self.mix_tail_quantile_cap)
					),
					'mix_tail_quantile_cap_x': (
						None if self.mix_tail_quantile_cap_x is None else float(self.mix_tail_quantile_cap_x)
					),
		...
		elif self.mix_tail_quantile_cap_x is not None:
			if not np.any(quantile_ok):
				self.setup_error = {
					'reason': 'no_quantile_cap_admissible_cutoff',
					'mix_tail_quantile_cap': float(self.mix_tail_quantile_cap),
					'mix_tail_quantile_cap_x': float(self.mix_tail_quantile_cap_x),
				}
				self.candidate_indices = np.array([], dtype=int)
				self.candidate_x = np.array([], dtype=float)
				self.x_count_pos = None
				return
			self.candidate_indices = self.candidate_indices[quantile_ok]
	...
	def setup_print(self, tag, max_checks, gap_tol, extra=""):
		...
		if self.setup_error is not None:
			...
			qcap = (
				""
				if self.mix_tail_quantile_cap is None
				else (
					f", mix_q_cap={self.mix_tail_quantile_cap:.3g}, "
					f"mix_q_cap_x={self.mix_tail_quantile_cap_x:.6g}"
				)
			)
			self.vprint(
				...
				f"tail_at_x_count=({mix_tail},{alt_tail}){qcap}"
			)
		else:
			mix_tail, alt_tail = self.tail_counts_at_pos(self.x_count_pos)
			self.vprint(
...
def alpha_minimize_fixed_derived_x0(
	...
	ctx = _DerivedSearchContext(
		...
		mix_tail_quantile_cap=mix_tail_quantile_cap,
...
def alpha_minimize_fixed_derived_x0_rightmost(
	...
	The CDF-validity and flat-tail levels default to the fixed values used in
	the reported pava3 experiments.  Passing ``None`` explicitly for either
	level couples that test to ``delta`` through the lower-level bootstrap plan.
	"""
...
def alpha_minimize_rightmost_delta_ladder(
	...
	paired-fallback convention.  By default only the residual-concavity and
	matched-convexity level changes across rungs; ``cdf_delta=0.01`` and
	``flat_delta=0.1`` remain fixed.  Passing ``None`` explicitly for either
	level couples that test to the current rung's delta.  The first successful
	rung is returned.  The return shape is the derived-x0 six-tuple:
...
def _candidate_thresholds_for_fdr(grid_x, x_mix, threshold_grid="fit_grid", candidate_thresholds=None):
	grid = np.asarray(grid_x, dtype=float)
	mix = np.asarray(x_mix, dtype=float)
	if threshold_grid not in {"fit_grid", "mixture_scores", "custom"}:
		raise ValueError("threshold_grid must be 'fit_grid', 'mixture_scores', or 'custom'")
	if threshold_grid == "custom" and candidate_thresholds is None:
		raise ValueError("candidate_thresholds must be supplied when threshold_grid='custom'")
	if threshold_grid == "fit_grid":
		thresholds = grid.copy()
	elif threshold_grid == "mixture_scores":
		thresholds = np.unique(mix)
	else:
		thresholds = np.asarray(candidate_thresholds, dtype=float)
		if thresholds.ndim != 1 or thresholds.size == 0:
			raise ValueError("candidate_thresholds must be a non-empty one-dimensional array")
		if not np.all(np.isfinite(thresholds)):
			raise ValueError("candidate_thresholds must contain only finite values")
		thresholds = np.unique(thresholds)
	return thresholds


def right_tail_alpha_cap(
	alpha_hat,
	grid_x,
	x_mix,
	x_alt,
	threshold_grid="fit_grid",
	candidate_thresholds=None,
	min_discoveries=1,
	lower_bound=None,
):
	"""
	Cap alpha so the raw estimated right-tail null fraction is nonnegative.

	For each candidate threshold x with enough observed mixture discoveries and
	positive matched tail mass, this computes Fn([x,0]) / Gm([x,0]).  The
	returned alpha_fdr is min(alpha_hat, min ratio).  The cap is intended for
	downstream FDR thresholding, not as a replacement for the main alpha fit.
	"""
	alpha = float(alpha_hat)
	if not np.isfinite(alpha) or alpha < 0.0 or alpha >= 1.0:
		raise ValueError(f"alpha_hat must be finite and in [0, 1), got {alpha_hat!r}")
	grid = np.asarray(grid_x, dtype=float)
	mix = np.asarray(x_mix, dtype=float)
	alt = np.asarray(x_alt, dtype=float)
	if grid.ndim != 1 or grid.size == 0:
		raise ValueError("grid_x must be a non-empty one-dimensional array")
	if mix.ndim != 1 or mix.size == 0:
		raise ValueError("x_mix must be a non-empty one-dimensional array")
	if alt.ndim != 1 or alt.size == 0:
		raise ValueError("x_alt must be a non-empty one-dimensional array")
	if min_discoveries < 0:
		raise ValueError("min_discoveries must be nonnegative")
	thresholds = _candidate_thresholds_for_fdr(
		grid, mix, threshold_grid=threshold_grid, candidate_thresholds=candidate_thresholds,
	)
	if lower_bound is not None:
		lower_bound = float(lower_bound)
		if not np.isfinite(lower_bound):
			raise ValueError("lower_bound must be finite when supplied")
		thresholds = thresholds[thresholds >= lower_bound]
		if thresholds.size == 0:
			return {
				"alpha_hat": alpha,
				"alpha_fdr": alpha,
				"alpha_tail_cap": np.nan,
				"tail_cap_active": False,
				"tail_cap_threshold": np.nan,
				"tail_cap_mix_tail": 0,
				"tail_cap_alt_tail": 0,
				"tail_cap_min_discoveries": int(min_discoveries),
				"tail_cap_lower_bound": lower_bound,
			}
	mix_sorted = np.sort(mix)
	alt_sorted = np.sort(alt)
	mix_left = np.searchsorted(mix_sorted, thresholds, side="left")
	alt_left = np.searchsorted(alt_sorted, thresholds, side="left")
	n_mix_tail = mix_sorted.size - mix_left
	n_alt_tail = alt_sorted.size - alt_left
	valid = n_mix_tail >= int(min_discoveries)
	valid &= n_alt_tail > 0
	if not np.any(valid):
		return {
			"alpha_hat": alpha,
			...
			"tail_cap_alt_tail": 0,
			"tail_cap_min_discoveries": int(min_discoveries),
			"tail_cap_lower_bound": np.nan if lower_bound is None else lower_bound,
		}
	ratios = (n_mix_tail[valid] / float(mix_sorted.size)) / (n_alt_tail[valid] / float(alt_sorted.size))
	thresholds_valid = thresholds[valid]
	mix_tail_valid = n_mix_tail[valid]
	alt_tail_valid = n_alt_tail[valid]
	j = int(np.argmin(ratios))
	cap = float(max(0.0, min(1.0, ratios[j])))
	alpha_fdr = float(min(alpha, cap))
	return {
		"alpha_hat": alpha,
		"alpha_fdr": alpha_fdr,
		"alpha_tail_cap": cap,
		"tail_cap_active": bool(alpha_fdr < alpha),
		"tail_cap_threshold": float(thresholds_valid[j]),
		"tail_cap_mix_tail": int(mix_tail_valid[j]),
		"tail_cap_alt_tail": int(alt_tail_valid[j]),
		"tail_cap_min_discoveries": int(min_discoveries),
		"tail_cap_lower_bound": np.nan if lower_bound is None else lower_bound,
	}
...
def fdr_thresholds(
	...
	"""
	Select right-tail score thresholds from a fitted mixture decomposition.

	This is the label-free decision step.  A threshold x calls scores S >= x
	discoveries.  The estimated false-discovery numerator is

	    (1 - alpha_hat) * (1 - H_hat(x-)),

	and the denominator is the observed mixture tail fraction at x.  If no
	grid threshold satisfies a requested q level, that q row is returned with
	selected=False and NaN threshold fields.  Candidate thresholds can be the
	fit grid, the observed mixture scores, or a supplied custom grid; H_hat can
	be evaluated as a left-continuous step function or by linear interpolation.
	"""
	if q_grid is None:
		q_grid = np.linspace(0.0, 0.25, 26)
	alpha = float(alpha_hat)
	if not np.isfinite(alpha) or alpha < 0.0 or alpha >= 1.0:
		raise ValueError(f"alpha_hat must be finite and in [0, 1), got {alpha_hat!r}")
	grid = np.asarray(grid_x, dtype=float)
	H = np.asarray(H_hat, dtype=float)
	mix = np.asarray(x_mix, dtype=float)
	q_vals = np.asarray(q_grid, dtype=float)
	if grid.ndim != 1 or H.ndim != 1 or grid.size != H.size:
		raise ValueError("grid_x and H_hat must be one-dimensional arrays of the same length")
	if grid.size == 0:
		raise ValueError("grid_x and H_hat must be non-empty")
	if mix.ndim != 1 or mix.size == 0:
		raise ValueError("x_mix must be a non-empty one-dimensional array")
	if not np.all(np.isfinite(grid)) or not np.all(np.isfinite(H)):
		raise ValueError("grid_x and H_hat must contain only finite values")
	if not np.all(np.isfinite(mix)):
		raise ValueError("x_mix must contain only finite values")
	if not np.all(np.isfinite(q_vals)) or np.any(q_vals < 0.0):
		raise ValueError("q_grid must contain finite nonnegative values")
	if min_discoveries < 0:
		raise ValueError("min_discoveries must be nonnegative")
	if cdf_eval not in {"step", "linear"}:
		raise ValueError("cdf_eval must be 'step' or 'linear'")
	...
	if cdf_eval == "step":
		left_idx = np.searchsorted(grid, thresholds, side="left") - 1
		H_eval = np.zeros_like(thresholds, dtype=float)
		valid_idx = left_idx >= 0
		H_eval[valid_idx] = H[left_idx[valid_idx]]
	else:
		H_eval = np.interp(thresholds, grid, H, left=0.0, right=float(H[-1]))
	H_eval = np.clip(H_eval, 0.0, 1.0)
	estimated_false_fraction = (1.0 - alpha) * np.maximum(0.0, 1.0 - H_eval)

	mix_sorted = np.sort(mix)
	left = np.searchsorted(mix_sorted, thresholds, side="left")
	n_discoveries = mix_sorted.size - left
	observed_discovery_fraction = n_discoveries / float(mix_sorted.size)
	estimated_fdr = np.full(thresholds.shape, np.inf, dtype=float)
	valid = n_discoveries >= int(min_discoveries)
	valid &= observed_discovery_fraction > 0.0
	estimated_fdr[valid] = estimated_false_fraction[valid] / observed_discovery_fraction[valid]

	rows = []
	for q in q_vals:
		pass_idx = np.flatnonzero(estimated_fdr <= float(q))
		if pass_idx.size == 0:
			rows.append({
				"q": float(q),
				"selected": False,
				"grid_index": None,
				"threshold": np.nan,
				"estimated_fdr": np.nan,
				"estimated_false_fraction": np.nan,
				"observed_discovery_fraction": np.nan,
				"n_discoveries": 0,
			})
			continue
		j = int(pass_idx[0])
		rows.append({
			...
			"estimated_fdr": float(estimated_fdr[j]),
			"estimated_false_fraction": float(estimated_false_fraction[j]),
			"observed_discovery_fraction": float(observed_discovery_fraction[j]),
			"n_discoveries": int(n_discoveries[j]),
		})
	...
	curve = {
		...
		"estimated_false_fraction": estimated_false_fraction,
		"observed_discovery_fraction": observed_discovery_fraction,
...
def _interval_width_weights(x):
	x = np.asarray(x, dtype=float)
	if x.ndim != 1 or x.size == 0:
		raise ValueError("x must be a non-empty one-dimensional grid")
	if x.size == 1:
		return np.ones(1, dtype=float)
	dx = np.diff(x)
	if np.any(dx < 0.0):
		raise ValueError("x must be sorted in nondecreasing order")
	...
	if np.any(w <= 0.0):
		positive = w[w > 0.0]
		floor = 1.0 if positive.size == 0 else float(np.min(positive))
		w = np.where(w > 0.0, w, floor)
	return w


def _weighted_isotonic_increasing(y, w):
	y = np.asarray(y, dtype=float)
	w = np.asarray(w, dtype=float)
	if y.ndim != 1 or w.ndim != 1 or y.size != w.size:
		raise ValueError("y and w must be one-dimensional arrays of the same length")
	if y.size == 0:
		return np.empty(0, dtype=float)
	if not np.all(np.isfinite(y)) or not np.all(np.isfinite(w)):
		raise ValueError("y and w must contain only finite values")
	if np.any(w <= 0.0):
	...
	for i, (yi, wi) in enumerate(zip(y, w)):
		...
		while len(levels) >= 2 and levels[-2] > levels[-1]:
			...
			new_level = (levels[-2]*weights[-2] + levels[-1]*weights[-1]) / new_weight
			levels[-2] = new_level
...
def cdf_isotonic_projection_on_grid(grid_x, y, force_right_endpoint=True, weights=None):
	"""
	Interval-width weighted L2 projection onto bounded nondecreasing grid CDFs.

	When force_right_endpoint is true, the final grid value is fixed to 1.
	This matches the score-grid convention used here, where the right endpoint
	is 0 and the CDF at 0 must be one.
	"""
	x = np.asarray(grid_x, dtype=float)
	y = np.asarray(y, dtype=float)
	if x.ndim != 1 or y.ndim != 1 or x.size != y.size:
		raise ValueError("grid_x and y must be one-dimensional arrays of the same length")
	if x.size == 0:
		raise ValueError("grid_x and y must be non-empty")
	if not np.all(np.isfinite(x)) or not np.all(np.isfinite(y)):
		raise ValueError("grid_x and y must contain only finite values")
	if weights is None:
		w = _interval_width_weights(x)
	else:
		w = np.asarray(weights, dtype=float)
		if w.ndim != 1 or w.size != x.size:
			raise ValueError("weights must be one-dimensional and match grid_x")
		if not np.all(np.isfinite(w)) or np.any(w <= 0.0):
			raise ValueError("weights must contain only finite positive values")
	if x.size == 1:
		return np.array([1.0 if force_right_endpoint else np.clip(y[0], 0.0, 1.0)], dtype=float)
	out = np.empty_like(y, dtype=float)
	if force_right_endpoint:
		out[:-1] = np.clip(_weighted_isotonic_increasing(y[:-1], w[:-1]), 0.0, 1.0)
		out[-1] = 1.0
	else:
		out[:] = np.clip(_weighted_isotonic_increasing(y, w), 0.0, 1.0)
...
def residual_cdf_on_grid(
	alpha_hat,
	grid_x,
	x_mix,
	x_alt,
	valid_cdf=True,
	projection_weight_mode="interval",
	projection_weight_gamma=0.0,
	projection_tail_floor=1e-3,
):
	"""
	Raw matched-component-subtracted residual CDF on an existing grid.

	This is the natural plug-in CDF for downstream FDR thresholding.  It is
	deliberately not the LCM/PAVA-fitted shape projection; that projection is a
	majorant used for shape testing and can understate right-tail null mass.
	When valid_cdf is true, the raw residual is projected onto the bounded
	nondecreasing CDF cone with interval-width weights and right endpoint 1.
	"""
	alpha = float(alpha_hat)
	if not np.isfinite(alpha) or alpha < 0.0 or alpha >= 1.0:
		raise ValueError(f"alpha_hat must be finite and in [0, 1), got {alpha_hat!r}")
	...
	if alt.ndim != 1 or alt.size == 0:
		raise ValueError("x_alt must be a non-empty one-dimensional array")
	if projection_weight_mode not in {"interval", "right_tail"}:
		raise ValueError("projection_weight_mode must be 'interval' or 'right_tail'")
	if projection_weight_gamma < 0.0 or not np.isfinite(projection_weight_gamma):
		raise ValueError("projection_weight_gamma must be finite and nonnegative")
	if projection_tail_floor <= 0.0 or not np.isfinite(projection_tail_floor):
		raise ValueError("projection_tail_floor must be finite and positive")
	Fn = _ecdf_on_grid(np.sort(mix), grid)
	Gm = _ecdf_on_grid(np.sort(alt), grid)
	R = (Fn - alpha*Gm) / max(1e-12, 1.0 - alpha)
	if valid_cdf:
		weights = None
		if projection_weight_mode == "right_tail":
			emphasis = np.power(np.maximum(float(projection_tail_floor), Fn), float(projection_weight_gamma))
			weights = _interval_width_weights(grid) * emphasis
		return cdf_isotonic_projection_on_grid(
			grid, R, force_right_endpoint=True, weights=weights
		)
	return R


def tail_proxy_alpha_correction(
	alpha_hat,
	x0_hat,
	H_hat,
	G_hat,
	...
	"""
	Compute a cheap one-sided tail-moment correction for a fitted alpha.

	The fitted component CDFs are held fixed.  At ``x0_hat`` the function
	simulates only mixture and matched-reference tail counts from

		F_tail* ~ Binomial(n, F_tail) / n,
		G_tail* ~ Binomial(m, G_tail) / m,

	and forms ``alpha_tail* = (F_tail* - H_tail) / (G_tail* - H_tail)``.
	The returned decision value subtracts the upper ``1 - gamma`` quantile of
	``alpha_tail* - alpha_hat``, truncated so the correction and result remain
	nonnegative.  This is a conditional fitted-model diagnostic, not a
	confidence bound for the full adaptive estimator.
	...
	B = int(B)
	gamma = float(gamma)
	denominator_eps = float(denominator_eps)
	min_valid_fraction = float(min_valid_fraction)
	if not np.isfinite(alpha) or alpha < 0.0 or alpha >= 1.0:
		raise ValueError(f"alpha_hat must be finite and in [0, 1), got {alpha_hat!r}")
	if not np.isfinite(x0):
		raise ValueError("x0_hat must be finite")
	if grid.ndim != 1 or H.ndim != 1 or G.ndim != 1:
		raise ValueError("grid_x, H_hat, and G_hat must be one-dimensional")
	if grid.size < 2 or H.size != grid.size or G.size != grid.size:
		raise ValueError("grid_x, H_hat, and G_hat must have the same length of at least two")
	if not np.all(np.isfinite(grid)) or not np.all(np.isfinite(H)) or not np.all(np.isfinite(G)):
		raise ValueError("grid_x, H_hat, and G_hat must contain only finite values")
	if np.any(np.diff(grid) <= 0.0):
		raise ValueError("grid_x must be strictly increasing")
	if x0 < grid[0] or x0 > grid[-1]:
		raise ValueError("x0_hat must lie within grid_x")
	if np.any(H < -1e-10) or np.any(H > 1.0 + 1e-10):
		raise ValueError("H_hat must lie in [0, 1]")
	if np.any(G < -1e-10) or np.any(G > 1.0 + 1e-10):
		raise ValueError("G_hat must lie in [0, 1]")
	if np.any(np.diff(H) < -1e-10) or np.any(np.diff(G) < -1e-10):
		raise ValueError("H_hat and G_hat must be nondecreasing")
	if n < 1 or m < 1:
		raise ValueError("n and m must be positive")
	if B < 100:
		raise ValueError("B must be at least 100")
	if not 0.0 < gamma < 0.5:
		raise ValueError("gamma must lie in (0, 0.5)")
	if not np.isfinite(denominator_eps) or denominator_eps <= 0.0:
	...
	h_tail = float(np.clip(1.0 - np.interp(x0, grid, H), 0.0, 1.0))
	g_tail = float(np.clip(1.0 - np.interp(x0, grid, G), 0.0, 1.0))
	tail_separation = g_tail - h_tail
	if tail_separation <= denominator_eps:
		raise ValueError(
			"the fitted matched tail must exceed the fitted background tail at x0_hat"
		)
	f_tail = h_tail + alpha * tail_separation
	if f_tail < -1e-12 or f_tail > 1.0 + 1e-12:
		raise ValueError("the fitted mixture tail probability lies outside [0, 1]")
	f_tail = float(np.clip(f_tail, 0.0, 1.0))

	rng = np.random.default_rng(random_state)
	f_star = rng.binomial(n, f_tail, size=B) / float(n)
	g_star = rng.binomial(m, g_tail, size=B) / float(m)
	denominator = g_star - h_tail
	valid = denominator > denominator_eps
	n_valid = int(np.sum(valid))
	min_valid = max(100, int(np.ceil(min_valid_fraction * B)))
	invalid_fraction = float(1.0 - n_valid / float(B))
	if n_valid < min_valid:
		raise RuntimeError(
			f"only {n_valid}/{B} tail-proxy replicates had a valid denominator; "
			f"at least {min_valid} are required"
		)

	alpha_star = (f_star[valid] - h_tail) / denominator[valid]
	alpha_star = np.clip(alpha_star, 0.0, 0.999999)
	error_star = alpha_star - alpha
	quantile = float(np.quantile(error_star, 1.0 - gamma))
	correction = float(max(0.0, quantile))
	alpha_decision = float(max(0.0, alpha - correction))
	seed_value = int(random_state) if isinstance(random_state, (int, np.integer)) else None
	info = {
		"success": True,
		"method": "fixed_x0_binomial_tail_proxy",
		"alpha_hat": alpha,
		"alpha_decision": alpha_decision,
		"correction": correction,
		"correction_active": bool(correction > 0.0),
		"gamma": gamma,
		"quantile_probability": 1.0 - gamma,
		"B": B,
		...
		"H_tail_x0": h_tail,
		"G_tail_x0": g_tail,
		"F_tail_model_x0": f_tail,
		"tail_separation": tail_separation,
		"proxy_sd": float(np.std(alpha_star, ddof=1)),
		"proxy_invalid_fraction": invalid_fraction,
		"proxy_valid_replicates": n_valid,
		"denominator_eps": denominator_eps,
		"min_valid_fraction": min_valid_fraction,
	}
	return alpha_decision, info


def _decision_residual_cdf(
	alpha_decision,
	x0_hat,
	grid_x,
	x_mix,
	x_alt,
	residual_cdf_mode="raw_clip0",
	projection_weight_mode="interval",
	projection_weight_gamma=0.0,
	projection_tail_floor=1e-3,
):
	"""Construct the residual curve paired with a downstream decision alpha."""
	grid = np.asarray(grid_x, dtype=float)
	if residual_cdf_mode not in {"projection", "shape", "raw_clip0"}:
		raise ValueError("residual_cdf_mode must be 'projection', 'shape', or 'raw_clip0'")
	if residual_cdf_mode == "projection":
		return residual_cdf_on_grid(
			alpha_decision,
			grid,
			x_mix,
			x_alt,
			...
			projection_weight_gamma=projection_weight_gamma,
			projection_tail_floor=projection_tail_floor,
		)
	raw = residual_cdf_on_grid(
		alpha_decision, grid, x_mix, x_alt, valid_cdf=False
	)
	if residual_cdf_mode == "raw_clip0":
		return np.maximum(raw, 0.0)
	return _as_valid_cdf(
		_concave_projection_on_interval(
			grid, raw, float(x0_hat), 0.0
		)
	)


def tail_proxy_adjustment(
	alpha_hat,
	x0_hat,
	H_hat,
	G_hat,
	...
	"""
	Apply the optional tail-proxy correction after ``alpha_minimize``.

	The original ``alpha_hat`` remains the point estimate.  The returned
	``alpha_decision`` and ``H_decision`` form a consistent pair for downstream
	FDR thresholding.  ``residual_cdf_mode`` accepts the same ``projection``,
	``shape``, and ``raw_clip0`` choices used by the simulation scripts.

	Returns
	-------
	alpha_decision, H_decision, info
	"""
	mix = np.asarray(x_mix, dtype=float)
	alt = np.asarray(x_alt, dtype=float)
	grid = np.asarray(grid_x, dtype=float)
	if mix.ndim != 1 or mix.size == 0 or not np.all(np.isfinite(mix)):
		raise ValueError("x_mix must be a non-empty one-dimensional finite array")
	if alt.ndim != 1 or alt.size == 0 or not np.all(np.isfinite(alt)):
		raise ValueError("x_alt must be a non-empty one-dimensional finite array")
	alpha_decision, info = tail_proxy_alpha_correction(
		alpha_hat,
		x0_hat,
		H_hat,
		G_hat,
	...
	H_decision = _decision_residual_cdf(
		alpha_decision,
		x0_hat,
		grid,
		mix,
		alt,
		residual_cdf_mode=residual_cdf_mode,
		projection_weight_mode=projection_weight_mode,
		projection_weight_gamma=projection_weight_gamma,
		projection_tail_floor=projection_tail_floor,
	)
	...
	info.update({
		"residual_cdf_mode": residual_cdf_mode,
		"projection_weight_mode": projection_weight_mode,
		"projection_weight_gamma": float(projection_weight_gamma),
		"projection_tail_floor": float(projection_tail_floor),
	})
	return alpha_decision, np.asarray(H_decision, dtype=float), info


TOOL_TAIL_PROXY_SETTINGS = {
	"casanovo": {
		"display_name": "Casanovo",
		"enabled": True,
		"gamma": 0.25,
	...
	"instanovo": {
		...
		"enabled": True,
		"gamma": 0.35,
		"quantile_probability": 0.65,
	},
	"deepnovo": {
		"display_name": "DeepNovo",
		"enabled": False,
		"gamma": None,
...
def _tool_tail_proxy_key(tool):
	if not isinstance(tool, str) or not tool.strip():
		raise ValueError("tool must be one of: Casanovo, InstaNovo, DeepNovo")
	key = "".join(char for char in tool.lower() if char.isalnum())
	if key not in TOOL_TAIL_PROXY_SETTINGS:
		supported = ", ".join(
			setting["display_name"] for setting in TOOL_TAIL_PROXY_SETTINGS.values()
		)
		raise ValueError(f"unknown tool {tool!r}; supported tools are {supported}")
	return key


def tool_tail_decision(
	tool,
	alpha_hat,
	x0_hat,
	H_hat,
	...
	denominator_eps=1e-10,
	min_valid_fraction=0.9,
	proxy_failure="fallback",
):
	"""Apply the current clean-data tool-specific tail decision.

	The calibrated settings are Q0.75 for Casanovo, Q0.65 for InstaNovo,
	and no proxy for DeepNovo.  The reported point estimate ``alpha_hat`` is
	never changed.  The returned ``alpha_decision`` and ``H_decision`` are the
	pair to use with :func:`fdr_thresholds`.

	By default, an expected proxy failure emits a warning and falls back to the
	unadjusted alpha with a consistently reconstructed residual curve.  Set
	``proxy_failure='raise'`` to propagate the original exception instead.

	Returns
	-------
	alpha_decision, H_decision, info
	"""
	key = _tool_tail_proxy_key(tool)
	setting = TOOL_TAIL_PROXY_SETTINGS[key]
	failure_mode = str(proxy_failure).strip().lower().replace("-", "_")
	if failure_mode not in {"fallback", "raise"}:
		raise ValueError("proxy_failure must be 'fallback' or 'raise'")

	common_info = {
		"tool": setting["display_name"],
		"tool_key": key,
		"calibration": TOOL_TAIL_PROXY_CALIBRATION,
		"proxy_enabled": bool(setting["enabled"]),
		"calibrated_gamma": setting["gamma"],
		"calibrated_quantile_probability": setting["quantile_probability"],
	}
	if not setting["enabled"]:
		...
		info = {
			"success": True,
			"method": "tool_calibrated_no_proxy",
			"alpha_hat": float(alpha_hat),
			"alpha_decision": float(alpha_hat),
			"correction": 0.0,
		...
		return float(alpha_hat), np.asarray(H_decision, dtype=float), info

	try:
		alpha_decision, H_decision, info = tail_proxy_adjustment(
			alpha_hat,
			x0_hat,
			H_hat,
			G_hat,
		...
		if failure_mode == "raise":
			raise
		warnings.warn(
			f"{setting['display_name']} tail proxy failed; using unadjusted alpha: {exc}",
			RuntimeWarning,
			stacklevel=2,
		)
		H_decision = _decision_residual_cdf(
		...
		info = {
			"success": False,
			"method": "tool_calibrated_tail_proxy_fallback",
			"alpha_hat": float(alpha_hat),
			"alpha_decision": float(alpha_hat),
			"correction": 0.0,
			"correction_active": False,
...
def alpha_minimize(
	...
	``shape_weight_gamma=6``.  The CDF-validity and flat-tail levels remain
	fixed across the ladder at ``cdf_delta=0.01`` and ``flat_delta=0.1``.
	Passing ``None`` explicitly for either level restores the older behavior in
	which that level is coupled to the current rung's delta.
	...
	if x0 is not None:
		...
		out_info.update({
			...
			'derived_options_unused': {
				...
				'mix_tail_quantile_cap': None if mix_tail_quantile_cap is None else float(mix_tail_quantile_cap),
				'x0_tol': float(x0_tol),
